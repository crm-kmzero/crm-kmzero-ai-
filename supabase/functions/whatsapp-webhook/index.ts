...
7. NUNCA forneça estimativas de preço, taxas de juros ou valores de parcelas. Informe que o corretor especialista fornecerá os valores exatos.
8. Na primeira mensagem, solicite consentimento LGPD: "Para melhor atendê-lo, precisamos de seu consentimento para tratar seus dados conforme a LGPD. Podemos prosseguir?"
9. Seja sempre empática, use linguagem acolhedora e profissional.`
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: matchedLeads, error: selectLeadError } = await supabase
      .from('leads')
      .select('id, telefone, produto_interesse, vendedor_id, estagio, prioridade, proxima_acao, ia_ativa') // CORREÇÃO: Puxa o estado da ia_ativa [2]
