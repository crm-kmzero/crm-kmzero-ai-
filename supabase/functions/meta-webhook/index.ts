...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    await supabase.from('leads').update({ produto_interesse: produto, vendedor_id: vendedorId, estagio, prioridade, proxima_acao: proximaAcao, ia_ativa: iaAtiva, canal_origem: parsed.channel, dados_cotacao: {}, ultimo_contato: new Date().toISOString() }).eq('id', lead.id)
